...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    install_argb_tools() {
     ...
     docker_option() {
               echo
               echo -e "${YELLOW}Do you wanna install docker? (y/n) ${RESET}"
               echo
               while true; do
                  read choice
                  if [[ "$choice" == 'y' || "$choice" == 'Y' ]]; then
     ...
     winboat_option() {
               echo
               echo -e "${YELLOW}Do you wanna install winboat? (y/n) ${RESET}"
               echo
               while true; do
                  read choice
                  if [[ "$choice" == 'y' || "$choice" == 'Y' ]]; then
                      msg 'Installing winboat...'
                      repo_docker
                      repo_update
                      install_docker
                      install_freerdp
                      install_winboat
                      msg 'Winboat installed successfully!'
                       break
                  fi
                  if [[ "$choice" == 'n' || "$choice" == 'N' ]]; then
                       break
...
EOF
    ...
    scripts_repository_common_begginer() {
        cd /
        mkdir -p /opt/tools
        cp -R /Scripts-Automation/scripts/tools/* /opt/tools/
        cd /opt/tools/
        rm -fr /opt/tools/scripts_pt-br
        stow scripts
