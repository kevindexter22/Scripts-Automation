...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    install_essentials_tools() {
        sudo apt install aptitude htop net-tools snapd gparted cpu-x gdebi git tesseract-ocr poppler-utils whois vim binutils preload default-jdk ubuntu-restricted-extras stow traceroute ssh dnsutils mtr iperf3 nload gnupg2 ca-certificates f3 tree gnome-calendar setserial cu liquidctl -y
    ...
    install_advanced_tools() {
       sudo apt install nmap alacritty tmux -y
    }
    
    install_basic_applications() {
        sudo apt install libreoffice audacity youtubedl-gui qbittorrent vlc winff handbrake thunderbird gnome-shell-extensions arandr -y
    ...
    install_design_applications() {
        sudo apt install gimp inkscape blender -y
    }
    ...
    install_cd_dvd_burn() {
        sudo aptitude install brasero k3b -y
    }
    install_programming_applications() {
         sudo apt install arduino arduino-* dbeaver-ce ca-certificates code android-tools-adb -y
     ...
     install_steam() {
          sudo aptitude install steam-installer steam-devices -y
     }
     install_lutris() {
          sudo aptitude install lutris -y
          flatpak install flathub net.davidrevoy.ProtonUp-Qt -y
          mkdir -p ~/.local/share/lutris/runners/wine
          flatpak override --user --filesystem=~/.local/share/lutris net.davidrevoy.ProtonUp-Qt
          flatpak info --show-permissions net.davidrevoy.ProtonUp-Qt | grep filesystems
     }
     install_retroarch() {
     ...
     install_video_editor_live() {
          sudo aptitude install obs-studio kdenlive -y
     }
     install_docker() {
          ...
          read -p 'Enter the username that you choose on system installation: ' user_docker
     ...
     install_winboat() {
        wget --max-redirect 100 https://github.com/TibixDev/winboat/releases/download/v0.9.0/winboat-0.9.0-amd64.deb
        sudo dpkg -i winboat*
     ...
     docker_option() {
               echo
               echo -e "${YELLOW}Você deseja instalar o docker? (s/n) ${RESET}"
               echo
               while true; do
                  read choice
                  if [[ "$choice" == 's' || "$choice" == 'S' ]]; then
                      ...
                      repo_update
                      install_docker
                      msg 'Docker instalado com sucesso!'
                       break
                  fi
                  if [[ "$choice" == 'n' || "$choice" == 'N' ]]; then
                       break
     ...
     winboat_option() {
               echo
               echo -e "${YELLOW}Você deseja instalar o winboat? (s/n) ${RESET}"
               echo
               while true; do
                  read choice
                  if [[ "$choice" == 's' || "$choice" == 'S' ]]; then
                      msg 'Instalando o winboat...'
                      repo_docker
                      repo_update
                      install_docker
                      install_freerdp
                      install_winboat
                      msg 'Winboat instalado com sucesso!'
                       break
                  fi
                  if [[ "$choice" == 'n' || "$choice" == 'N' ]]; then
                       break
     ...
     gpu_install_opt() {
        echo
         ...
         echo '3 - Intel HD Graphics'
         echo '4 - Intel Iris Xe | Arc'
         echo 's - Pular ou sair'
         echo
         while true; do
            read -p "Digite sua opção: " gpu_vendor
             ...
             for choice in $gpu_vendor; do
                ...
                1)
                    ...
                    sudo apt install -y mesa-vulkan-drivers mesa-vulkan-drivers:i386 libgl1-mesa-dri libgl1-mesa-dri:i386 mesa-utils vulkan-tools rocm-opencl-runtime corectrl lm-sensors
                ...
                3)
                   ...
                   sudo apt install -y intel-media-va-driver i965-va-driver-shaders mesa-va-drivers intel-gpu-tools vainfo
                   msg 'GPU Drivers installed successfully!'
                   ;;
                4) 
                   msg 'Installing Intel Iris Xe | Arc Drivers...'
     ...
     install_for_gamers() {
         ...
         echo '4 - Moonlight'
         echo '5 - Gerenciador de iluminacao aRGB'
            ...
            read -p "Digite suas escolhas (separadas com espaço, ex: 2 3 5): " game
             ...
             for choice in $game; do
                ...
                4)
                    ...
                    ;;
                5)
                    msg 'Installing Open aRGB...'
                    repo_open_argb
                    install_argb_tools
                    msg 'Open aRGB installed successfully!'
                    ;;
                6)
       ...
       install_extra_packages_options() {
         ...
         echo '8 - Instalar o winboat'
         echo '9 - Instalar o docker'
         echo 's - Pular ou sair'
         echo
         while true; do
...
# ─────────────────────────────────────────────────────────────────────────────
     dotfiles_repository_begginer_common() {
        git clone https://github.com/kevindexter22/Scripts-Automation.git /Scripts-Automation
        echo
        ...
        mkdir -p /home/$user_common/dotfiles
        mkdir -p /home/$user_common/.fonts
        sudo cp -R /Scripts-Automation/archives/dotfiles/* /home/$user_common/dotfiles
        sudo cp -R /Scripts-Automation/archives/.fonts/* /home/$user_common/.fonts
        chown -R $user_common:$user_common /home/$user_common/dotfiles
        chown -R $user_common:$user_common /home/$user_common/.fonts
...
EOF
    ...
    scripts_repository_common_begginer() {
        cd /
        mkdir -p /opt/scripts
        cp -R /Scripts-Automation/scripts/tools/scripts/* /opt/scripts
        rm -fr /opt/tools
    ...
    desktop_repository_common_begginer() {
        cd /
        cd /usr/share
        cp -R /Scripts-Automation/archives/scripts_desktop/* /usr/share/applications/
        cd /
	cd /opt
        mkdir -p /opt/.icon
        cp -R /Scripts-Automation/archives/.icon/* /opt/.icon/
	cd /
...
EOF
    ...
    scripts_repository_advanced_all() {
        cd /
        mkdir -p /opt/scripts
        cp -R /Scripts-Automation/scripts/tools/scripts_pt-br/* /opt/scripts
        rm -fr /opt/tools
        sudo chmod +x /opt/scripts/*.sh
        msg 'Você irá encontrar os scripts na pasta /opt/scripts.'
        msg 'Configuração aplicada com sucesso!'    
